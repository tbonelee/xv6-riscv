//
// Tests for copy-on-write vfork() - this version should SUCCEED
// This test allocates more than half of available physical memory,
// then calls vfork(). The vfork should succeed using COW.
//
...
simple_test()
{
  ...
  printf("cowtest_vfork: simple test (should SUCCEED)\n");
  ...
  // Now try to vfork - this should succeed using COW
  printf("\nAttempting vfork() with %d MB allocated...\n", ALLOCATE_MB);
  
  int free_pages_before_vfork = freelistcount();
  printf("Free pages before vfork(): %d\n", free_pages_before_vfork);
  
  pid = vfork();
  if (pid < 0) {
    printf("simple: vfork() failed (UNEXPECTED)\n");
    printf("COW implementation may have issues\n");
    
    int free_pages_after_failed_vfork = freelistcount();
    printf("Free pages after failed vfork(): %d\n", free_pages_after_failed_vfork);
    printf("Pages difference due to failed vfork: %d\n", free_pages_before_vfork - free_pages_after_failed_vfork);
    exit(1);
  } else if (pid == 0) {
    // Child process
    printf("simple: vfork() succeeded - child process\n");
    ...
    printf("Pages consumed by vfork (COW setup): %d\n", free_pages_before_vfork - free_pages_in_child);
...
{
  printf("=== COW Test with vfork() (should succeed) ===\n");
  printf("This test demonstrates that vfork() uses copy-on-write\n");
